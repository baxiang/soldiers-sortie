...
func (admin *APPController) APPList(c *gin.Context) {
	...
	for _, item := range list {
		appCounter, err := pkg.FlowCounterHandler.GetCounter(pkg.FlowAppPrefix + item.AppID)
		if err != nil {
...
func (admin *APPController) AppAdd(c *gin.Context) {
	...
	if params.Secret == "" {
		params.Secret = pkg.MD5(params.AppID)
	}
...
func (admin *APPController) AppStatistics(c *gin.Context) {
	...
	//今日流量全天小时级访问统计
	todayStat := []int64{}
	counter, err := pkg.FlowCounterHandler.GetCounter(pkg.FlowAppPrefix + detail.AppID)
	if err != nil {
		middleware.ResponseError(c, 2002, err)
		c.Abort()
		return
