...
func (service *ServiceController) ServiceList(c *gin.Context) {
	...
	for _, listItem := range list {
		...
		if serviceDetail.Info.LoadType == pkg.LoadTypeHTTP &&
			serviceDetail.HTTPRule.RuleType == pkg.HTTPRuleTypePrefixURL &&
		...
		if serviceDetail.Info.LoadType == pkg.LoadTypeHTTP &&
			serviceDetail.HTTPRule.RuleType == pkg.HTTPRuleTypeDomain {
			serviceAddr = serviceDetail.HTTPRule.Rule
		}
		if serviceDetail.Info.LoadType == pkg.LoadTypeTCP {
			serviceAddr = fmt.Sprintf("%s:%d", clusterIP, serviceDetail.TCPRule.Port)
		}
		if serviceDetail.Info.LoadType == pkg.LoadTypeGRPC {
			serviceAddr = fmt.Sprintf("%s:%d", clusterIP, serviceDetail.GRPCRule.Port)
		}
		ipList := serviceDetail.LoadBalance.GetIPListByModel()
		counter, err := pkg.FlowCounterHandler.GetCounter(pkg.FlowServicePrefix + listItem.ServiceName)
		if err != nil {
...
func (service *ServiceController) ServiceStat(c *gin.Context) {
	...
	counter, err := pkg.FlowCounterHandler.GetCounter(pkg.FlowServicePrefix + serviceDetail.Info.ServiceName)
	if err != nil {
		middleware.ResponseError(c, 2004, err)
		return
	}
